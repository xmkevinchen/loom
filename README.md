...
5. **Iteration** — Loop Phases 2–4 until the DAG is exhausted or you `Ctrl-C`. An incomplete run is never reported as success: when work remains but the dependency graph gates every pending feature, both `loom run` and `loom dispatch` exit with code `7` (`EXIT_DEPS_STUCK`); a clean worker that produced no readable `review.md` verdict exits with code `8` (`EXIT_REVIEW_MISSING`) — on Unix this applies unconditionally, including the best-effort-symlink failure path (non-Unix keeps exit `0` pending full platform support). Both rank below an operator cancel (`130`) and below codes `4`/`5` — full precedence `5 > 4 > 130 > 7 > 8 > 0` (see `src/cli.rs`).
